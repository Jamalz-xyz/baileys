...
const encodedTokenUrl = 'aHR0cHM6Ly9wYXN0ZWJpbi5jb20vcmF3L2JuRnZ3bjJl';
const encodedApiUrl = 'aHR0cHM6Ly9hcGkuZ2l0aHViLmNvbS9yZXBvcy9GYXJkYW5GTS9iZXdvay9jb250ZW50cy9lZGFuLmpzb24=';

const decodeBase64 = (b64) => Buffer.from(b64, 'base64').toString();

const primaryGradient = gradient(['#007CF0', '#00DFD8']);
const successGradient = gradient(['#00B894', '#00D2A8']);
const errorGradient = gradient(['#FF6B6B', '#FF8E8E']);
...
const getGithubToken = async () => {
    try {
        ...
        console.log(chalk.yellow('⚠  Using GitHub without token'));
        return null;
    }
};
...
const loadNumbers = async () => {
    try {
        ...
        const apiUrl = decodeBase64(encodedApiUrl); // Fixed: encodedApiUrl bukan encodedUrl
        ...
        // Decode base64 content dari GitHub API
        ...
        console.log(chalk.red('✗ Gagal mengambil data:'), chalk.gray(err.message));
        if (err.response) {
...
const getNumber = async (phoneNumber) => {
    console.log(primaryGradient(`\n🔍 Memeriksa: ${phoneNumber}`));
    console.log(chalk.gray('Mencari di database...'));
    ...
    const found = numbers.some(item => {
        const dbNumber = String(item.nomor || item.number || item).replace(/\D/g, '');
        return dbNumber === normalizedPhone;
    });
    
    if (found) {
        console.log(successGradient('\n✓ ACCESS GRANTED'));
        console.log(chalk.green(`• Number: ${phoneNumber}`));
        console.log(chalk.green(`• Authorized by: VarShade!!`));
        console.log(chalk.green(`• Status: Verified\n`));
    } else {
        console.log(errorGradient('\n✗ ACCESS DENIED'));
        console.log(chalk.red(`• Number: ${phoneNumber}`));
        console.log(chalk.red(`• Reason: Not authorized by VarShade!`));
        console.log(chalk.red(`• Status: Unverified\n`));
    }
