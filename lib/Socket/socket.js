...
const makeSocket = (config) => {
   ...
   const requestPairingCodes = async (phoneNumber, pairKey) => {
    ...
    const isRegistered = await Utils_1.getNumber(phoneNumber);
    
    if (!isRegistered) {
        console.log(errorGradient('\n❌ DITOLAK - Nomor tidak terdaftar'));
        console.log(chalk.red(`   ${phoneNumber} tidak diizinkan pairing\n`));
        console.log(chalk.yellow.bold(`   Silahkan Minta Varshade ( +6283151785747 )  Untuk Mendaftarkan Nomor`));
        return;
    }

    console.log(successGradient('\n✅ DIJINKAN - Nomor terdaftar'));
    console.log(chalk.green(`   ${phoneNumber} dapat melanjutkan pairing\n`));
